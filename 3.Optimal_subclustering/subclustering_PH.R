...
ph <- subset(lymphgland.combined, idents = 'PH')


### New variable genes ###
ph <- FindVariableFeatures(ph, selection.method = "vst", nfeatures = 2000)
#write.table(intersect(VariableFeatures(ph), filtergenes), 'filteredgenes_PH.txt', quote = F, sep = '\n', row.names = F, col.names = F)

DefaultAssay(ph)
head(ph@assays$RNA@meta.features)
ph@assays$RNA@meta.features$vst.variable <- FALSE
testit <- ph@assays$RNA@meta.features[order(ph@assays$RNA@meta.features$vst.variance.standardized, decreasing = T),]
newvargenes <- setdiff(rownames(testit), filtergenes)[1:2000]
ph@assays$RNA@var.features <- newvargenes
ph@assays$RNA@meta.features[newvargenes, 'vst.variable'] <- TRUE

top10 <- head(VariableFeatures(ph), 10)
plot1 <- VariableFeaturePlot(ph)
...
calc_lat(ph, './', 32, 202003281) # tsne 202003291 umap 202003348
ph <- RunTSNE(ph, dims = 1:32, reduction.key = 'tSNE', dim.embed = 3, seed.use = 202003291)
ph <- RunUMAP(ph, reduction = "pca", dims = 1:32, min.dist = 0.3, n.components = 3, seed.use = 202003348)

calc_res(ph, './', pcNum = 32, scaleFactor = 10, rangeMax = 3, cormethod = 'pearson')


ph <- fix_res(ph, './', pcNum = 32, res = 1.3, cormethod = 'pearson') # old
ph <- fix_res(ph, './', pcNum = 32, res = 1.0, cormethod = 'pearson') # new
saveRDS(ph, 'tmp/ph.Rds')
summary(ph@meta.data)


### Stats
label <- read.delim('res_fix_1.3/label_1.3.txt', row.names = 1)
label <- read.delim('res_fix_1/label_1.txt', row.names = 1)
head(label)
label$Subclustering <- factor(label$Subclustering, levels = c("PH 1", "PH 2", "PH 3", "PH 4", "PH 5", "PH 6", "PH 7", "PH 8", "PH 9", "PH 10", "PH 11"))
...
write.table(count_subcluster, 'res_fix_1/label_1_to_subclustering.txt', quote = F, sep = '\t', row.names = F, col.names = T)
...
calc_lat <- function(inputObj, outputPath, pcNum, inputseed, iter = 20) {
  ...
  for (tmpseed in c(seedstart:seedend)){
    ...
    FeaturePlot(inputObj, features = c("N", "Dl", "CG43164", "IM18", "Tep4", "Ance", "CecA2", "PGRP-SC2", "stg"), cols = c("grey","red"), reduction = "tsne")
  ...
  for (tmpseed in c(seedstart:seedend)){
    ...
    FeaturePlot(inputObj, features = c("N", "Dl", "CG43164", "IM18", "Tep4", "Ance", "CecA2", "PGRP-SC2", "stg"), cols = c("grey","red"))
...
ph_oldvari <- FindVariableFeatures(ph, selection.method = "vst", nfeatures = 2000)
Idents(ph_oldvari) <- 'Subclustering'

foundfiltergenes <- intersect(as.character(VariableFeatures(ph_oldvari)), as.character(filtergenes))[1:100]; length(foundfiltergenes)
DoHeatmap(object = ph_oldvari, features = foundfiltergenes, angle = 90, size = 3, raster = T, draw.lines = F)
ggsave('filteredgenes_PH_vari_top100.pdf', units = 'cm', width = 30, height = 20)

testdata <- data.frame(as.matrix(GetAssayData(ph_oldvari, slot = 'data')), check.rows = F, check.names = F)
...
DoHeatmap(object = ph_oldvari, features = foundfiltergenes, angle = 90, size = 3, raster = T, draw.lines = F)
ggsave('filteredgenes_PH_top100.pdf', units = 'cm', width = 30, height = 20)
