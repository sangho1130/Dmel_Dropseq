...
DimPlot(blood.combined_flt, dims = c(2, 3))
...
plt <- ggplot(avg_ph, aes(LG, Circ, label = labelgenes, col = colorgenes)) + 
  geom_point() + scale_color_manual(values = c('#69b9c2', '#fd9409', 'grey90')) + 
  ggtitle("blood.combined_flt") + xlim(0, 8) + ylim(0, 8) + 
  geom_text_repel(data = avg_ph[c(top10$gene[1:5]), ], color = '#fd9409',
                  nudge_x = 8 - avg_ph[c(top10$gene[1:5]), ]$LG,
                  segment.size = 0.2, segment.color = "grey50", direction = "y", hjust = 1) +
  geom_text_repel(data = avg_ph[top10$gene[6:14], ], 
                  nudge_y = 8 - avg_ph[top10$gene[6:14], ]$Circ,
                  segment.size = 0.2, segment.color = "grey50", direction = "x", vjust = 0, angle = 90) +
  theme_bw() +
  theme(legend.position = 'None', panel.grid = element_blank(), plot.title = element_blank());plt
...
ggsave('compare/ph4/blood.combined_flt_scatter.PH4.augment.png', units = 'cm', width = 4, height = 4)
...
plt <- ggplot(avg_pm, aes(LG, Circ, label = labelgenes, col = colorgenes)) + 
  geom_point() + scale_color_manual(values = c('#69b9c2', '#fd9409', 'grey90')) + 
  ggtitle("blood.combined_flt") + xlim(0, 8) + ylim(0, 8) + 
  geom_text_repel(data = avg_pm[c(top10$gene[1:7]), ], color = '#fd9409',
                  nudge_x = 8 - avg_pm[c(top10$gene[1:7]), ]$LG,
                  segment.size = 0.2, segment.color = "grey50", direction = "y", hjust = 1) +
  geom_text_repel(data = avg_pm[top10$gene[8:17], ], 
                  nudge_y = 8 - avg_pm[top10$gene[8:17], ]$Circ,
                  segment.size = 0.2, segment.color = "grey50", direction = "x", vjust = 0, angle = 90) +
  theme_bw() +
  theme(legend.position = 'None', panel.grid = element_blank(), plot.title = element_blank());plt
