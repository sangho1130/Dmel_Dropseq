...
blood.combined <- readRDS('tmp/blood.combined.Rds')
head(blood.combined@meta.data)

circ_only <- subset(blood.combined, cells = rownames(subset(blood.combined@meta.data, timepoint == "Circ_96"|timepoint == "Circ_120")))
...
blood.combined@meta.data$origin <- mapvalues(blood.combined@meta.data$timepoint, 
                                             ...
                                             to = c('LG', 'LG', 'Circ', 'Circ'))
summary(subset(blood.combined@meta.data, origin == 'LG' & timepoint == 'LG_96')$Subclustering)
summary(subset(blood.combined@meta.data, origin == 'LG' & timepoint == 'LG_120')$Subclustering)
...
#DimPlot(blood.combined_flt, dims = c(2,3), split.by = 'origin')
...
writeLabel <- data.frame(Barcode = rownames(blood.combined_flt@meta.data), blood.combined_flt@meta.data, check.rows = F, check.names = F)
write.table(writeLabel, 'label_filtered.txt', quote = F, sep = '\t', row.names = F, col.names = T)
